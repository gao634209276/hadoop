...
public class TopNSorted2 {
	...
	public static class DataMapper extends
		...
		protected void setup(
			...
			for (int i = 0; i < 4; i++) {
				System.out
						.println("map setup:  " + i + " =i,topN   " + topN[i]);
		...
		protected void cleanup(Context context) throws IOException,
			...
			for (int i = 1; i < length + 1; i++) {
				context.write(new Text(String.valueOf(topN[i])), new Text(
						String.valueOf(topN[i])));
			}
		}
	...
	public static class DataReducer extends Reducer<Text, Text, Text, Text> {
		...
		protected void setup(Reducer<Text, Text, Text, Text>.Context context)
			...
			topN = new int[length + 1];
			for (int i = 0; i < 4; i++) {
				System.out.println("Reducer setup:  " + i + " =i,topN   "
						+ topN[i]);
		...
		protected void cleanup(Context context) throws IOException,
			...
			for (int i = length; i > 0; i--) {
				System.out.println("reduce cleanup:  " + i + " =i,topN   "
						+ topN[i]);
				context.write(new Text(String.valueOf(length - i + 1)),
						new Text(String.valueOf(topN[i])));
			}
		}
	}
