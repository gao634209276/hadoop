...
public class logSession {
	...
	static class sessionReducer extends Reducer<Text, Text, Text, NullWritable> {
		...
		protected void reduce(Text key, Iterable<Text> values, Context context)
			...
			for (Text browseHistory : values) {
				WebLogSessionBean sessionBean = sessionParser
						.loadBean(browseHistory.toString());
				sessionBeanGroup.add(sessionBean);
			}
			Collections.sort(sessionBeanGroup,
					new Comparator<WebLogSessionBean>() {

						public int compare(WebLogSessionBean sessionBean1,
								WebLogSessionBean sessionBean2) {
			...
			for (WebLogSessionBean sessionBean : sessionBeanGroup) {
				...
				if (sessionStartTime == null) {
					...
					long sessionStayTime = timeDiffer(sessionStartTime,
							sessionEndTime);
	...
	public static void main(String[] args) throws Exception {
		...
		// 指定job的输入原始文件所在目录
		FileInputFormat.setInputPaths(job, new Path("/clickstream/cleandata/"
				+ dateStr + "/*"));
		// 指定job的输出结果所在目录
		FileOutputFormat.setOutputPath(job, new Path(
				"/clickstream/sessiondata/" + dateStr + "/"));
