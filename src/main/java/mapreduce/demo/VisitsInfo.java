...
public class VisitsInfo {
	...
	public static class visitReducer extends
		...
		protected void reduce(Text key, Iterable<Text> values, Context context)
			...
			Collections.sort(browseInfoGroup, new Comparator<String>() {
				...
				public int compare(String browseInfo1, String browseInfo2) {
					String dateStr1 = browseInfo1.split(" ")[0] + " "
							+ browseInfo1.split(" ")[1];
					String dateStr2 = browseInfo2.split(" ")[0] + " "
							+ browseInfo2.split(" ")[1];
					Date date1;
					Date date2;
					try {
			...
			String visitsInfo = visitsParser.parser(browseInfoGroup,
					total_visit_pages + "");
	...
	public static void main(String[] args) throws Exception {
		...
		FileInputFormat.setInputPaths(job, new Path("/clickstream/sessiondata/"
				+ dateStr + "/*"));
		// 指定job的输出结果所在目录
		FileOutputFormat.setOutputPath(job, new Path("/clickstream/visitsinfo"
				+ dateStr + "/"));
