...
public class VisitsInfoParser {
	...
	public String parser(ArrayList<String> pageViewsGroup, String totalVisitNum) {
		...
		String leavePage = pageViewsGroup.get(pageViewsGroup.size() - 1).split(
				" ")[4];
		String startTime = pageViewsGroup.get(0).split(" ")[0] + " "
				+ pageViewsGroup.get(0).split(" ")[1];
		String endTime = pageViewsGroup.get(pageViewsGroup.size() - 1).split(
				" ")[0]
				+ " "
